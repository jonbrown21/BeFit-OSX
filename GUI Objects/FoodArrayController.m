...
- (IBAction) servingSizeChanged:(id) sender
{
	...
	if( sender == mPopupButton )
	{
		...
		if( indexOfSelectedItem != formerIndex )
		{
            ...
            [ mCaloriesFieldPerc setStringValue: [ NSString stringWithFormat: @"%ld", [ food caloriesLongValue ]]];
            ...
            [ mTotalFatPercentageField setStringValue: [ food totalFatPercent ]];
			[ mSugarField setStringValue: [ food sugarsValue ]];
			[ mProteinField setStringValue: [ food proteinValue ]];
            [ mProteinFieldPerc setStringValue: [ food proteinValuePerc ]];
            [ mIronField setStringValue: [ food ironValue ]];
            [ mCalcField setStringValue: [ food calciumValue ]];
            [ mVitCField setStringValue: [ food vitaminCValue ]];
            [ mIronFieldPerc setStringValue: [ food ironValuePerc ]];
            [ mVitCFieldPerc setStringValue: [ food vitaminCValuePerc ]];
            [ mCalcFieldPerc setStringValue: [ food calciumValuePerc ]];
            [ mSugarFieldPerc setStringValue: [ food sugarsValuePerc ]];
...
- (BOOL)setSelectionIndexes:(NSIndexSet *)indexes
{
	...
	if( [ indexes count ] == 1 )
	{
		...
		{			
			...
			{
                if ([servingAmountValue  isEqual: @"1 ITEM"]) {
                    [mPopupButton setHidden: YES];
                } else {
