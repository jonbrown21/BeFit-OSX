...
+ (void)drawBackgroundInRect:(NSRect)rect hilighted:(BOOL)hilighted
{
    ...
    CGRectDivide(rect, &borderRect, &fillRect, 1.0, CGRectMaxYEdge);
    ...
    NSRect	_dividerRect = NSMakeRect(rect.origin.x + rect.size.width - 1, 3, 1,(rect.size.height - 7));
    NSRectFill(_dividerRect);
    
    //[self drawInteriorWithFrame:CGRectInset(fillRect, 0.0, 1.0) inView:rect];
